...
function useInsights() {
  ...
  return useMemo(() => {
    ...
    if (historicalOpen.length > 0) {
      focus.push({ label: 'Revisar historico', detail: `${historicalOpen.length} pendente${historicalOpen.length > 1 ? 's' : ''} anterior${historicalOpen.length > 1 ? 'es' : ''}`, href: '/daily', color: '#f59e0b', icon: AlertTriangle })
    }
    if (focus.length === 0) {
      focus.push({ label: 'Fluxo em dia', detail: 'Sem risco imediato', href: '/daily', color: '#10b981', icon: CheckCircle2 })
    }
    ...
    if (criticalQA.length > 0) {
      insights.push({
        ...
        metric: `${criticalQA.length} critico${criticalQA.length > 1 ? 's' : ''}`,
    ...
    insights.push({
      ...
      detail: `${doneTasks.length} de ${tasks.length} tarefas concluidas hoje.`,
      action: completionPct >= 80 ? 'Abrir Daily' : 'Priorizar Daily',
      href: '/daily',
    })
    ...
    if (criticalTasks.length > 0) {
      insights.push({
        ...
        detail: `${criticalTasks.length} tarefa${criticalTasks.length > 1 ? 's' : ''} critica${criticalTasks.length > 1 ? 's' : ''} pendente${criticalTasks.length > 1 ? 's' : ''} no Daily.`,
        action: 'Abrir Daily',
        href: '/daily',
      })
    } else {
      const byClient = openTasks.reduce<Record<string, number>>((acc, task) => {
      ...
      insights.push({
        ...
        title: 'Carga do dia',
        detail: topClient
          ? `${openTasks.length} abertas. Maior foco: ${topClient[0]} com ${topClient[1]} tarefa${topClient[1] > 1 ? 's' : ''}.`
          : 'Nenhuma tarefa aberta para hoje.',
        action: openTasks.length > 0 ? 'Balancear foco' : 'Abrir Daily',
        href: '/daily',
      })
    }
...
export function SentinelAI() {
  const { insights, focus } = useInsights()
        ...
        {focus.map(item => {
          ...
          return (
            <button
              ...
              className="flex items-center gap-3 rounded-xl border border-white/[0.07] bg-white/[0.03] px-3 py-2.5 text-left hover:border-white/[0.12] hover:bg-white/[0.05]"
            >
              <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg" style={{ backgroundColor: `${item.color}18`, color: item.color }}>
          ...
          return (
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2 mb-3 pl-1">
                ...
                <span className="text-xs font-bold tabular-nums leading-none mt-0.5" style={{ color: insight.color }}>
                  {insight.metric}
                </span>
              </div>
              ...
              <div
                className="flex items-center gap-1 mt-2.5 pl-1 text-[10px] font-medium max-h-0 overflow-hidden opacity-0 transition-all duration-200 ease-out group-hover:max-h-6 group-hover:opacity-100"
